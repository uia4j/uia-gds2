...
extends GDSRecord {
   ...
   */
  public GDSReflibsRecord(GDSRecord rec)
   ...
   *                              specified.
   */
  public GDSReflibsRecord(String reflibs[])
    throws GDSRecordException {
    setReflibs(reflibs);
   ...
   */
  public GDSReflibsRecord(short length, byte rectype, byte dattype,
      byte data[])
    throws GDSRecordException {
    ...
    if((data.length % 44) != 0) {
      throw new GDSRecordException(i18n.getString(
          i18n.i18n_REFLIBS_THROW1));
    }

    int    numlibs    = (int)(data.length / 44);
  ...
  public String[] getReflibs() {
    String result[] = new String[reflibs.length];
    System.arraycopy(reflibs, 0, result, 0, reflibs.length);
  ...
  public String toString() {
    String result = i18n.getString(i18n.i18n_REFLIBS_TOSTRING1);

    for(int i = 0;i < reflibs.length;i++) {
      result += GDSStringUtil.sprintf(i18n.getString(
            i18n.i18n_REFLIBS_TOSTRING2), i, reflibs[i]);
    }
  ...
  private byte[] updateData()
    ...
    for(int i = 0;i < reflibs.length;i++) {
      try {
        baos.write(GDSSpecificDataConverter.writeJavaString(reflibs[i],
            44));
      } catch(IOException e) {
   ...
   *                              44 characters
   */
  private String[] validate(String reflibs[])
    throws GDSRecordException {
    if((reflibs.length < 2) || (reflibs.length > 15)) {
      throw new GDSRecordException(i18n.getString(
          i18n.i18n_REFLIBS_THROW2));
    }

    if(reflibs[0].length() < 1) {
      throw new GDSRecordException(i18n.getString(
          i18n.i18n_REFLIBS_THROW3));
    }

    for(int i = 0;i < reflibs.length;i++) {
      if(reflibs[i].length() > 44) {
        throw new GDSRecordException(i18n.getString(
            i18n.i18n_REFLIBS_THROW4));
      }
    }
