...
extends GDSRecord {
   ...
   */
  public GDSLibsecurRecord(short length, byte rectype, byte dattype,
      byte data[])
    throws GDSRecordException {
    ...
    for(int i = 0;i < (data.length / 6);i++) {
      ...
      if(!libsecur.contains(elem)){libsecur.add(elem);}
    }
  }
  ...
  public GDSAccessControl[] getLibsecur() {
    ...
    for(int i = 0;i < result.length;i++) {
      result[i] = new GDSAccessControl(libsecur.get(i));
    }
  ...
  public String toString() {
    String result = i18n.getString(i18n.i18n_LIBSECUR_TOSTRING1);

    for(int i = 0;i < libsecur.size();i++) {
      result += GDSStringUtil.sprintf(i18n.getString(
            i18n.i18n_LIBSECUR_TOSTRING2), i,
          libsecur.get(i).toString());
    }
