...
public class GDSSpecificDataConverter {
  ...
  public static List flattenList(List flat, List hier) {
    if((flat == null) || (hier == null)){return new ArrayList();}
    for(int i = 0;i < hier.size();i++) {
      if(!(hier.get(i) instanceof java.util.List)){flat.add(hier.get(i));}
      else if(((java.util.List)hier.get(i)).size() == 0) {
        continue;
      } else{flat = flattenList(flat, (java.util.List)hier.get(i));}
    }
   ...
   */
  public static ByteArrayOutputStream fromJavaDouble(double data,
   ...
   */
  public static ByteArrayOutputStream fromJavaFloat(float data,
      ByteArrayOutputStream baos) {
   ...
   */
  public static ByteArrayOutputStream fromJavaString(String data,
      ByteArrayOutputStream baos) {
   ...
   */
  public static ByteArrayOutputStream fromJavaString(String data, int len,
      ByteArrayOutputStream baos) {
  ...
  public static byte[] writeJavaString(String data, int len) {
    ...
    if(data.length() > (len - 1)){reg = data.substring(0, len - 1);}
    else{reg = data;}
