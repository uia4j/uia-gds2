 ...
 * @since    1.5
 */
public class GDSFontsRecord extends GDSRecord {
     ...
     */
    public GDSFontsRecord(GDSRecord rec) throws GDSRecordException {
     ...
     *                              mapping files.
     */
    public GDSFontsRecord(String fonts[]) throws GDSRecordException {
        setFonts(fonts);
        this.rectype = FONTS;
     ...
     */
    public GDSFontsRecord(int length, byte rectype, byte dattype, byte data[]) throws GDSRecordException {
    ...
    public String[] getFonts() {
        String result[] = new String[this.fonts.length];
        System.arraycopy(this.fonts, 0, result, 0, this.fonts.length);
     ...
     *                              mapping files.
     */
    public void setFonts(String fonts[]) throws GDSRecordException {
    ...
    public String toString() {
        String result = GDSI18NFactory.getString(GDSI18NFactory.i18n_FONTS_TOSTRING1);

        for (int i = 0; i < this.fonts.length; i++) {
            result += GDSStringUtil.sprintf(GDSI18NFactory.getString(
                    GDSI18NFactory.i18n_FONTS_TOSTRING2), i, this.fonts[i]);
        }
    ...
    private String[] validate(String fonts[]) throws GDSRecordException {
        ...
        for (int i = 0; i < fonts.length; i++) {
            ...
            if (fonts[i].length() > 44) {
                throw new GDSRecordException(GDSI18NFactory.getString(
                        GDSI18NFactory.i18n_FONTS_THROW2));
            }
        }

        if (noneDefined) {
            throw new GDSRecordException(GDSI18NFactory.getString(
                    GDSI18NFactory.i18n_FONTS_THROW3));
        }
