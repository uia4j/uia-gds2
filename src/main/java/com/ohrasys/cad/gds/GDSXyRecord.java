...
extends GDSRecord {
   ...
   */
  public GDSXyRecord(GDSRecord rec)
   ...
   */
  public GDSXyRecord(short length, byte rectype, byte dattype, byte data[])
    throws GDSRecordException {
    super(length, rectype, dattype, data);
    validateIntRec(XY, MAX_REC_LEN, 8);

    if((data.length % 8) != 0) {
      throw new GDSRecordException(i18n.getString(i18n.i18n_XY_THROW1));
    }

    ByteArrayInputStream bais       = new ByteArrayInputStream(data);
    Point                register[] = new Point[(int)(data.length / 8)];
  ...
  public void setXy(Point xy[])
    throws GDSRecordException {
    if(xy.length < 1) {
      throw new GDSRecordException(i18n.getString(i18n.i18n_XY_THROW2));
    }
  ...
  public String toString() {
    String result = new String(i18n.getString(i18n.i18n_XY_TOSTRING1));
    result += " " /*NOI18N*/;

    for(int i = 0;i < xy.length;i++) {
      result += GDSStringUtil.sprintf(i18n.getString(
            i18n.i18n_XY_TOSTRING2), xy[i].x, xy[i].y);
