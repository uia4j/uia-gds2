 ...
 * @since    1.5
 */
public class GDSTapecodeRecord extends GDSRecord {
     ...
     */
    public GDSTapecodeRecord(GDSRecord rec) throws GDSRecordException {
     ...
     */
    public GDSTapecodeRecord(short tapecode[]) throws GDSRecordException {
     ...
     */
    public GDSTapecodeRecord(int length, byte rectype, byte dattype, byte data[]) throws GDSRecordException {
    ...
    public short[] getTapecode() {
        short result[] = new short[this.tapecode.length];
        System.arraycopy(this.tapecode, 0, result, 0, this.tapecode.length);
     ...
     * @throws  GDSRecordException  If the tapecode doesnt contain 6 elements.
     */
    public void setTapecode(short code[]) throws GDSRecordException {
        if (code.length != 6) {
            throw new GDSRecordException(GDSI18NFactory.getString(
                    GDSI18NFactory.i18n_TAPECODE_THROW));
        }

        this.tapecode = new short[6];
        System.arraycopy(code, 0, this.tapecode, 0, code.length);
    ...
    public String toString() {
        String result = GDSI18NFactory.getString(GDSI18NFactory.i18n_TAPECODE_TOSTRING1);

        for (int i = 0; i < this.tapecode.length; i++) {
            result += GDSStringUtil.sprintf(GDSI18NFactory.getString(
                    GDSI18NFactory.i18n_TAPECODE_TOSTRING2), i, this.tapecode[i]);
        }
    ...
    private byte[] updateData() {
        ...
        for (int i = 0; i < this.tapecode.length; i++) {
            baos = GDSByteConverter.fromShort(this.tapecode[i], baos);
        }
